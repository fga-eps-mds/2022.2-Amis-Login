...
print("Salve: ", str(Path(__file__).resolve().parents[3]))
sys.path.append(str(Path(__file__).resolve().parents[3]))

from sqlalchemy.orm import Session
from fastapi import HTTPException, status
from src.domain.models.student import Status
...
def test_save_student(mock_db_session: Session):
...
def test_find_all_student(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
...
def test_find_by_login_student(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
...
def test_exists_by_cpf_student(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
...
def test_delete_by_login_student(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
...
def test_update_by_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    student = Student(**test_student_data)
...
def test_save_student_existing_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    student = Student(**test_student_data)
...
def test_find_by_login_student_existing_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
...
def test_exists_by_cpf_student_existing_cpf(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    existing_student = Student(**test_student_data)
...
def test_delete_by_login_student_existing_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    existing_student = Student(**test_student_data)
    existing_student.login = "existinguser"
    mock_db_session.first = lambda: existing_student
...
def test_update_by_login_existing_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    existing_student = Student(**test_student_data)
    existing_student.login = "existinguser"
    mock_db_session.first = lambda: existing_student
...
def test_save_student_without_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    student = Student(**test_student_data)
    student.login = None
...
def test_find_by_login_student_nonexistent_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
...
def test_exists_by_cpf_student_nonexistent_cpf(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    mock_db_session.first = lambda: None
...
def test_update_by_login_nonexistent_login(mock_db_session: Session):
    student_repo = StudentRepository(mock_db_session)
    mock_db_session.first = lambda: None

    with pytest.raises(HTTPException) as exc:
        student_repo.update_by_login(Student(**test_student_data))
